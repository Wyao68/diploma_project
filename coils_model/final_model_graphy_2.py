...
def build_input_grid(n:int,
    ...
    s_unique = len(s_list)
    w_unique = len(w_list)
    s_grid = np.array(s_grid).reshape(s_unique, w_unique)
    w_grid = np.array(w_grid).reshape(s_unique, w_unique)   
    return X, s_grid, w_grid
...
def main():
    ...
    # 构建输入数据
    n = 5
    w_vals = np.arange(0.1, 3.0, 0.01).tolist()
    d_out = 70.0
    s_vals = np.arange(0.1, 3.0, 0.01).tolist()
    angle = 30.0
    
    X_norm, s_grid, w_grid = build_input_grid(n=n, d_out=d_out, w_list=w_vals, s_list=s_vals, angle=angle, mean=x_mean, std=x_std)
    ...
    # 重塑为网格形状 (s_unique, w_unique)
    s_unique = len(s_vals)
    w_unique = len(w_vals)
    Q_grid = Q.reshape(s_unique, w_unique)
    
    # 绘制三维曲面图
    fig = plt.figure(figsize=(10, 7))
    ax = fig.add_subplot(111, projection='3d')
    surf = ax.plot_surface(s_grid, w_grid, Q_grid, cmap='viridis', edgecolor='none')
    ax.set_xlabel('Conductor Spacing (mm)')
    ...
    ax.text2D(0.05, 0.95, f"Number of Turns: {n}, Outer Diameter: {d_out} mm, Angle: {angle}°", transform=ax.transAxes)
    max_Q = np.max(Q_grid)
    max_idx = np.unravel_index(np.argmax(Q_grid), Q_grid.shape)
    ax.text2D(0.05, 0.90, f"Max Q: {max_Q:.2f} at s={s_grid[max_idx]:.2f} mm, w={w_grid[max_idx]:.2f} mm", transform=ax.transAxes)
